    ...
    def diagnose(self, input_values, observed_values, max_faults=20, max_steps=1000, use_gnn=True, max_time_seconds=180):
        ...
        def run_search(active_score_map, deadline, branch_limit):
            ...
            while open_set:
                ...
                cost, g, current_faults = heapq.heappop(open_set)
                ...
                for node in valid_candidates[:branch_limit]:
                    ...
                    for val in [0, 1]:
                        ...
                        if new_faults not in visited:
                            new_g = len(new_faults)
                            new_total_cost = cost + step_cost
                            heapq.heappush(open_set, (new_total_cost, new_g, new_faults))
...
def evaluate_astar_performance(circuit_name, bench_path, gnn_model, num_test_samples=20, num_injected_faults=2, timeout_seconds=180, gnn_only=False):
    ...
    for case_idx in range(num_test_samples):
        ...
        with open(obs_path, "w", encoding="utf-8") as f:
            ...
            f.write("# inputs\n")
            for node in sorted(input_vec.keys()):
    ...
    print(
        ...
        f"Avg Card: {gnn_avg_card:.2f}, Avg Time: {gnn_total_time / num_test_samples:.3f}s, "
...
def main():
    ...
    if should_train:
        ...
        train_fault_probs = None
        
        num_nodes = len(G.nodes())
        if num_nodes > 2000:
            num_samples = 1000
            print(f"Large circuit detected ({num_nodes} nodes). Reducing training samples to {num_samples}.")
            # Large-circuit default: include higher-cardinality faults.
            if train_fault_counts == [1, 2]:
                train_fault_counts = [1, 2, 5, 10]
                train_fault_probs = [0.4, 0.3, 0.2, 0.1]
        elif num_nodes > 1000:
            num_samples = 800            
            if train_fault_counts == [1, 2]:
                train_fault_counts = [1, 2, 5, 10]
                train_fault_probs = [0.4, 0.3, 0.2, 0.1]
            print(f"Medium circuit detected ({num_nodes} nodes). Reducing training samples to {num_samples}.")
        else:
            num_samples = 500
            if train_fault_counts == [1, 2]:
                train_fault_counts = [1, 2, 5, 10]
                train_fault_probs = [0.4, 0.3, 0.2, 0.1]
            print(f"Small circuit detected ({num_nodes} nodes). Using {num_samples} training samples.")
